...
static int __init ccs_init(void)
{
	if (!ccs_find_security_ops() || !ccs_find_find_task_by_pid() ||
		...
		printk(KERN_INFO "Sorry, we couldn't resolve dependent "
		       "symbols.\n");
		printk(KERN_INFO "We need some changes for supporting your "
		       "environment.\n");
		printk(KERN_INFO "Please contact the author.\n");
		return -EINVAL;
	}
	printk(KERN_INFO "All dependent symbols have been resolved.\n");
	printk(KERN_INFO "Please compare these addresses with System.map for "
	       "this kernel (e.g. /boot/System.map-`uname -r` ).\n");
	printk(KERN_INFO "If their addresses are correct, you can try loading "
