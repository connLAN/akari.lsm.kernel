 ...
 *           | half restricted |  always      | always          | always
 *           | restricted      |  always      | always          | same uuid
 *
...
static void uuid_print_uuid(const u8 uuid[UUID_SIZE],
			    char uuid_buf[UUID_PRINT_SIZE])
{
	snprintf(uuid_buf, UUID_PRINT_SIZE, "%08x-%04x-%04x-%04x-%04x%08x",
		 htonl(* (u32 *) uuid), htons(* (u16 *) (uuid + 4)),
		 htons(* (u16 *) (uuid + 6)), htons(* (u16 *) (uuid + 8)),
		 htons(* (u16 *) (uuid + 10)), htonl(* (u32 *) (uuid + 12)));
}
...
static bool uuid_check_task(struct task_struct *task, const char *func)
{
	...
	int ret = 0;
	char uuid_buf_current[UUID_PRINT_SIZE];
	if (task == current)
		return 0;
	rcu_read_lock();
	uuid_current = uuid_find_security(current_cred());
	if (!uuid_current || !uuid_current->uuid_configured)
		goto ok;
	uuid_task = uuid_find_security(__task_cred(task));
	if (uuid_task && (!uuid_task->uuid_configured ||
			  uuid_eq(uuid_task->uuid, uuid_current->uuid)))
		goto ok;
	uuid_print_uuid(uuid_current->uuid, uuid_buf_current);
	if (uuid_task) {
		char uuid_buf_task[UUID_PRINT_SIZE];
		uuid_print_uuid(uuid_task->uuid, uuid_buf_task);
		printk(KERN_INFO "Prevented task(%s) ('%s',pid=%u) from "
		       "accessing task(%s) ('%s',pid=%u) at %s\n",
		       uuid_buf_current, current->comm, current->pid,
		       uuid_buf_task, task->comm, task->pid, func);
	} else
		printk(KERN_INFO "Prevented task(%s) ('%s',pid=%u) from "
...
static int uuid_check_socket(struct socket *sock, const char *func)
{
	...
	printk(KERN_INFO "Prevented task(%s) ('%s',pid=%u) from accessing "
	       "socket(%s) at %s\n", uuid_buf_current, current->comm,
	       current->pid, uuid_buf_task, func);
	ret = -EPERM;
ok:
	rcu_read_unlock();
	return ret;
	...
	if (S_ISSOCK(inode->i_mode) || S_ISFIFO(inode->i_mode)) {
		...
		if (uuid_current && uuid_current->uuid_configured) {
			...
			if (uuid_task && uuid_task->uuid_configured) {
				char uuid_buf_current[UUID_PRINT_SIZE];
				char uuid_buf_task[UUID_PRINT_SIZE];
				uuid_print_uuid(uuid_current->uuid,
						uuid_buf_current);
				uuid_print_uuid(uuid_task->uuid,
						uuid_buf_task);
			  ...
			  loff_t *ppos)
{
	...
	{
		char uuid_buf_current[UUID_PRINT_SIZE];
		uuid_print_uuid(ptr->uuid, uuid_buf_current);
