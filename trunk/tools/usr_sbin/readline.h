...
again:
	...
	if (c == 127 || c == 8)
		c = KEY_BACKSPACE;
	/* syslog(LOG_INFO, "akari_getch0='%c' (%d)\n", c, c); */
	if (c == '\r' || c == '\n') {
		if (enter_key == EOF)
			enter_key = c;
		else if (c != enter_key)
...
static int akari_getch2(void)
{
	...
	/* syslog(LOG_INFO, "akari_getch2='%c'\n", c2); */
	switch (c2) {
	case '1':
		return KEY_HOME;
	case '2':
...
static int akari_add_history(const char *buffer, const char **history,
...
static char *akari_initial_readline_data = NULL;

static char *akari_readline(const int start_y, const int start_x, const char *prompt,
			  ...
			  const int max_length, const int scroll_width)
{
	...
	if (akari_initial_readline_data) {
		strncpy(buffer, akari_initial_readline_data, max_length);
